...
export default function InventoryHomeScreen({ navigation }) {
  ...
  const fetchAllPlaysWithMaterials = async () => {
    try {
      const res = await axios.get("http://192.168.0.103:5000/api/plays");
  ...
  const fetchInventoryItems = async () => {
    try {
      const res = await axios.get("http://192.168.0.103:5000/api/items");
  ...
  const fetchOrders = async () => {
    try {
      const res = await axios.get("http://192.168.0.103:5000/api/orders");
  ...
  const checkInventoryAvailability = async (materialsWithQuantities) => {
    try {
      const inventoryRes = await axios.get("http://192.168.0.103:5000/api/items");
      ...
      Alert.alert("Confirm Processing", "Are you sure you want to mark these materials as processing? This will reserve them from inventory.", [
        { text: "Cancel", style: "cancel" },
        {
          text: "Process", style: "default", onPress: async () => {
            try {
              await axios.patch(`http://192.168.0.103:5000/api/plays/${playId}/material-requests/${requestId}/processing`);
      ...
      Alert.alert("Confirm Preparation", "Are you sure you want to mark these materials as prepared? This will deduct them from inventory.", [
        { text: "Cancel", style: "cancel" },
        {
          text: "Prepare", style: "default", onPress: async () => {
            try {
              await axios.patch(`http://192.168.0.103:5000/api/plays/${playId}/material-requests/${requestId}/prepare`);
    ...
    try {
      ...
      const deductionPromises = materialsWithQuantities.map(async (requestedMaterial) => {
        ...
        if (inventoryItem) {
          ...
          await axios.put(`http://192.168.0.103:5000/api/items/${inventoryItem._id}`, { ...inventoryItem, quantity: newQuantity });
        }
      });
  ...
  const addNewItem = async () => {
    ...
    try {
      await axios.post("http://192.168.0.103:5000/api/items", newItem);
  ...
  const createOrder = async () => {
    ...
    try {
      ...
      await axios.post("http://192.168.0.103:5000/inventory/orders", orderPayload);
